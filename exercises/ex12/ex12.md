...
The final value is correct; however, the print statements are not.
...
The threads are running concurrently cuz the print statements are printing the same counter values which means more than one threads is accessing the counter variable at the same time. So yeah that is a synchronization error.
...
The synchronization imposes about 0.118s of overhead.
